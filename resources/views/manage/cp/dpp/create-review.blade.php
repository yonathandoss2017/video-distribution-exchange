...
@push('title')
    {{ __('app.title') }} | {{ __('manage/cp/ivideoadd/ivideoadd.review_ivideoadd_videos') }} — {{ $playlist->name }}
@endpush
...
<div class="container">
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div class="title-header ">
                        <div class="title">{{ __('manage/cp/ivideoadd/ivideoadd.review_ivideoadd_videos') }} — {{ $playlist->name }}</div>
                    </div>
                    <div class="ibox">
                        ...
                        <div class="video-list dpp-video">
                            ...
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th class="image"></th>
                                        <th class="video-name fx-video-name">{{ __('manage/cp/ivideoadd/ivideoadd.title') }}</th>
                                        <th class="duration">{{ __('manage/cp/ivideoadd/ivideoadd.duration') }}</th>
                                        <th class="text-right">{{ __('manage/cp/ivideoadd/ivideoadd.action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="image">
                                            <div class="video-img">
                                                <a href="#"><img src="{{ \App\Services\Serve\VideoImageService::getImageUrl($video, $video->property_id, null, 240) }}"></a>
                                            </div>
                                        </td>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-5">
                                    <div class="dataTables_info">
                                        ...
                                        {{
                                            __(
                                                'manage/cp/ivideoadd/ivideoadd.video_pagination',
                                                [
                ...
                @if (count($videoIds) > 0)
                <div class="form-save"><button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#submit">{{ __('manage/cp/ivideoadd/ivideoadd.submit_for_ivideoadd') }}</button></div>
                @endif
            </div>
        </div>
            ...
            <div class="modal-dialog s-width" role="document">
                <div class="modal-content">
                 <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('manage/cp/ivideoadd/ivideoadd.submit_for_ivideoadd') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                    <div>{{ __('manage/cp/ivideoadd/ivideoadd.enable_ivideoadd_for_your_videos') }}</div>
                    </div>
                    <div class="modal-footer">
                        {!! Form::open([
                                'method'=>'POST',
                                'url' => route('manage.cp.dpp.store', [ $property_id]),
                            ...
                            <button type="submit" class="btn btn-secondary">{{ __('manage/cp/ivideoadd/ivideoadd.submit') }}</button>
                        {{ Form::close() }}
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog s-width" role="document">
                <div class="modal-content">
                 <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('manage/cp/ivideoadd/ivideoadd.confirm_remove_video') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                    <div>{{ __('manage/cp/ivideoadd/ivideoadd.do_you_want_to_remove_this_video') }}?</div>
                    </div>
                    <div class="modal-footer">
                        <input type="hidden" name="video-delete-id" id="video-delete-id">
                        <button type="submit" class="btn btn-secondary" onclick="deleteVideo();">{{ __('manage/cp/ivideoadd/ivideoadd.delete') }}</button>
                    </div>
                </div>
            </div>
        </div>
