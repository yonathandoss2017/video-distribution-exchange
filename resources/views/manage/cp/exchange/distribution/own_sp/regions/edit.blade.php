...
@push('title')
    {{ __('app.title') }} | {{ __('manage/cp/exchange/distribution.edit_regions') }}
@endpush
            ...
            <div class="col-md-8 offset-md-2">
                <div class="form">
                    <div class="title-header ">
                        ...
                        <div class="title">{{ __('manage/cp/exchange/distribution.edit_regions') }}</div>
                    </div>
                    {!! Form::open(['route' => ['manage.cp.exchange.distribution.regions.update', $property_id, $id, $distribution_region->id], 'method' => 'PUT', 'class'=>'form-horizontal']) !!}
                        ...
                        <div id="term_info">
                            @include('manage.cp.exchange.distribution.regions.term', ['marketplaceTerm' => $distribution_region, 'isEdit' => true, 'show_date' => false])
                        </div>
                        ...
                        <div class="ibox">
                            <div class="ibox-title">
                                <h5>{{ __('manage/cp/exchange/distribution.sync_to_marketplace_terms') }}</h5>
                            </div>
                            <div class="ibox-content">
                                <div class="form-group row">
                                    <label class="col-md-3 control-label">{{ __('manage/cp/exchange/distribution.sync_or_not') }}</label>
                                    <div class="col-md-9">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <label class="custom-control custom-radio">
                                                    <input type="radio" name="sync_to_marketplace_terms" value="1" class="custom-control-input">
                                            ...
                                            <div class="col-md-4">
                                                <label class="custom-control custom-radio">
                                                    <input type="radio" name="sync_to_marketplace_terms" value="0" checked class="custom-control-input">
                                                    <span class="custom-control-indicator"></span>
...
@push('js')
    <script>
        ...
        function getMarketplaceTerm(obj)
        {
            var selVal = $(obj).val();
            $.ajax({
                url : request_url + "?term=" + selVal + "&show_date=false",
                dataType: 'html',
            }).done(function (data) {
                $('#term_info').html(data);
            }).fail(function () {
